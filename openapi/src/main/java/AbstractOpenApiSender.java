...
public abstract class AbstractOpenApiSender {
    ...
    public String call(OpenAPIRequest request) throws IOException, CipherException, RequestException {
        ...
        try (Response response = client.newCall(builder.build()).execute()) {
            ...
            switch (responseCode) {
                case RequestTest.ResponseHttpCode.OK:
                    return responseBody;
...
//                    }
                case RequestTest.ResponseHttpCode.BAD_REQUEST:
                    // 业务逻辑失败,do something
                    dealBadRequest(responseBody, request);
                    break;
                case RequestTest.ResponseHttpCode.NOT_FOUND:
                    // 资源不存在,do something
                    dealNotFundRequest(responseBody, request);
                    break;
                case RequestTest.ResponseHttpCode.UNAUTHORIZED:
                    // 客户端未授权或验签失败,do something
                    dealUnauthorized(responseBody, request);
                    break;
                case RequestTest.ResponseHttpCode.METHOD_NOT_ALLOWED:
                    // 方法不支持,do something
                    dealMethodNotAllowed(responseBody, request);
                    break;
                case RequestTest.ResponseHttpCode.INTERNAL_SERVER_ERROR:
                    // 内部服务器异常，联系xx以获得支持
